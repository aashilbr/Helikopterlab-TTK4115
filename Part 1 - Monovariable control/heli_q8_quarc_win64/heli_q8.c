 ...
 * C source code generated on : Tue Nov 17 10:48:29 2020
 *
...
void heli_q8_output0(void)             /* Sample time: [0.0s, 0.0s] */
{
  ...
  if (rtmIsMajorTimeStep(heli_q8_M)) {
    ...
    if (heli_q8_B.RateTransitionx > heli_q8_P.DeadZonex_End) {
      rtb_Sum_k = heli_q8_B.RateTransitionx - heli_q8_P.DeadZonex_End;
    } else if (heli_q8_B.RateTransitionx >= heli_q8_P.DeadZonex_Start) {
      rtb_Sum_k = 0.0;
    } else {
      rtb_Sum_k = heli_q8_B.RateTransitionx - heli_q8_P.DeadZonex_Start;
    }
     ...
     *  Gain: '<S4>/Gain: x'
     */
    heli_q8_B.Joystick_gain_x = heli_q8_P.Gainx_Gain * rtb_Sum_k *
    ...
    rtb_TmpSignalConversionAtToFile[6] = heli_q8_B.Joystick_gain_x;

    /* ToFile: '<Root>/To File' */
    if (rtmIsMajorTimeStep(heli_q8_M)) {
      {
        if (!(++heli_q8_DW.ToFile_IWORK.Decimation % 1) &&
            (heli_q8_DW.ToFile_IWORK.Count*8)+1 < 100000000 ) {
          ...
          if (fp != (NULL)) {
            ...
            if (fwrite(u, sizeof(real_T), 8, fp) != 8) {
              rtmSetErrorStatus(heli_q8_M,
                                "Error writing to MAT-file Day1-Tuning.mat");
              return;
            }

            if (((++heli_q8_DW.ToFile_IWORK.Count)*8)+1 >= 100000000) {
              (void)fprintf(stdout,
                            "*** The ToFile block will stop logging data before\n"
                            "    the simulation has ended, because it has reached\n"
                            "    the maximum number of elements (100000000)\n"
                            "    allowed in MAT-file Day1-Tuning.mat.\n");
            }
          }
        }
      }
  ...
  if (rtmIsMajorTimeStep(heli_q8_M)) {
    ...
    /* Sum: '<S2>/Sum' */
    rtb_Sum_k = heli_q8_B.Joystick_gain_y - heli_q8_B.Sum;
   ...
   *  Sum: '<S6>/Sum'
   */
  heli_q8_B.Sum_j = ((heli_q8_X.Integrator_CSTATE + heli_q8_B.K_ep) -
   ...
   *  Sum: '<S1>/Add'
   */
  u0 = (heli_q8_B.Sum_j - rtb_Backgain) * heli_q8_P.Frontgain_Gain;
   ...
   *  Sum: '<S1>/Subtract'
   */
  u0 = (heli_q8_B.Sum_j + rtb_Backgain) * heli_q8_P.Backgain_Gain;
...
void heli_q8_initialize(void)
{
  ...
  {
    FILE *fp = (NULL);
    char fileName[509] = "Day1-Tuning.mat";
    if ((fp = fopen(fileName, "wb")) == (NULL)) {
      rtmSetErrorStatus(heli_q8_M, "Error creating .mat file Day1-Tuning.mat");
      return;
    }

    if (rt_WriteMat4FileHeader(fp,8,0,"Tuning")) {
      rtmSetErrorStatus(heli_q8_M,
                        "Error writing mat file header to file Day1-Tuning.mat");
      return;
    }
...
void heli_q8_terminate(void)
{
  ...
  /* Terminate for ToFile: '<Root>/To File' */
  {
    ...
    if (fp != (NULL)) {
      char fileName[509] = "Day1-Tuning.mat";
      if (fclose(fp) == EOF) {
        rtmSetErrorStatus(heli_q8_M, "Error closing MAT-file Day1-Tuning.mat");
        return;
      }

      if ((fp = fopen(fileName, "r+b")) == (NULL)) {
        rtmSetErrorStatus(heli_q8_M, "Error reopening MAT-file Day1-Tuning.mat");
        return;
      }

      if (rt_WriteMat4FileHeader(fp, 8, heli_q8_DW.ToFile_IWORK.Count, "Tuning"))
      {
        rtmSetErrorStatus(heli_q8_M,
                          "Error writing header for Tuning to MAT-file Day1-Tuning.mat");
      }
...
RT_MODEL_heli_q8_T *heli_q8(void)
{
  ...
  rtmSetTFinal(heli_q8_M, 30.0);
  heli_q8_M->Timing.stepSize0 = 0.002;
  heli_q8_M->Timing.stepSize1 = 0.002;
  heli_q8_M->Timing.stepSize2 = 0.01;

  /* External mode info */
  heli_q8_M->Sizes.checksums[0] = (1316999875U);
  heli_q8_M->Sizes.checksums[1] = (2581389679U);
  heli_q8_M->Sizes.checksums[2] = (2356016439U);
  heli_q8_M->Sizes.checksums[3] = (1427258729U);
